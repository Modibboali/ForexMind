...
def run_diagnosis(
    ...
    # PPO first
    print("Evaluating PPO (frozen policy)...")
...
def print_summary(result: dict[str, Any]) -> None:
    ...
    header = (
        f"{'Agent':<18} {'Return':<12} {'Sharpe':<12} {'Sortino':<12} "
        f"{'Max DD':<12} {'Turnover':<12} {'Trades':<10}"
    )
    print(header)
    print("-" * 120)
    ...
    # PPO
    print(
        f"{'ppo':<18} {ppo_metrics.get('total_return', 0):<12.6f} "
        f"{ppo_metrics.get('sharpe', 0):<12.4f} {ppo_metrics.get('sortino', 0):<12.4f} "
        f"{ppo_metrics.get('max_drawdown_pct', 0):<12.6f} "
        f"{ppo_metrics.get('turnover', 0):<12.6f} "
    ...
    for baseline_name in BASELINE_AGENTS:
        ...
        print(
            f"{baseline_name:<18} {baseline_metrics.get('total_return', 0):<12.6f} "
            f"{baseline_metrics.get('sharpe', 0):<12.4f} "
            f"{baseline_metrics.get('sortino', 0):<12.4f} "
            f"{baseline_metrics.get('max_drawdown_pct', 0):<12.6f} "
            f"{baseline_metrics.get('turnover', 0):<12.6f} "
            f"{0:<10}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory (default: data/reports/ppo_diagnosis).",
    )
